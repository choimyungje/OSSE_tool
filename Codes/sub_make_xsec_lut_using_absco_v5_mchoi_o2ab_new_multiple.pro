PRO sub_make_xsec_lut_using_absco_v5_mchoi_o2ab_new_multiple, workspace_pwd, i_loop_start, input_loop_begin, wn_interval_read  ;, wn_range, wn_res


print, 'Process: interpolation of ABSCO table...'


; PRO make_xsec_lut_using_absco_v5_mchoi, 	 						 $
; 		INPUT_PATH, atmos_filename, wn_range, wn_res			   ;; INPUT

;input_path = '../PTH_PROFILES'
;atmos_filename = '/atmos_mchoi.dat'
...
; atmos_file = '../PTH_PROFILES/atmos.dat'
; atmos_file = '../PTH_PROFILES/atmos_mchoi.dat'
; workspace_pwd = '/home/mchoi/OSSE_tool'
input_loop=0
RTM_pwd = workspace_pwd+'/RTM/RTM_LAB/'+'GEMSTOOL_mchoi_LAB_loop'+string(input_loop+i_loop_start+input_loop_begin,f='(i3.3)')
atmos_file = RTM_pwd+'/GEMSTOOL_physicsdata/PTH_PROFILES/atmos_mchoi.dat'

;path  = '/home/mchoi/GEMSTOOL_mchoi_20181105_iron/GEMSTOOL_physicsdata/LUT_Xsections_data' 
; path2 = './results_files_V500'
path2 = RTM_pwd+'/GEMSTOOL_physicsdata/LUT_Xsections_data/results_files_V500'
spawn, 'rm -rf '+path2
file_mkdir, path2
...
; print, atmos_file
	...
	for i = 0, nlevels-1 do begin 
		...
		im_h2o(i)   = dum3
		im_o2(i)    = dum4
		; print, im_press(i), im_temp(i), im_h2o(i), im_o2(i)
	endfor  
	close,1
...
; wn_range = [ [12920.0000, 13230.0000], [6140.9400, 6317.5600], [4779.9600, 4922.3800] ]

gas = [ 'h2o', 'o2']
gas2 = [ 'H2O', 'O2']
index = [ '01', '07']
nxsecs = [ 91004, 56002 ]
; crs_wn_range = [ [12920, 13230], [14340, 14590]]
; wn_range = [ [12920.0000, 13230.0000], [14340.0000, 14590.0000] ]
crs_wn_range = [ [12920, 13230], [14340, 14590]]
wn_range = [ [12920.0000, 13230.0000], [14340.0000, 14590.0000] ]

for ngas = 0, n_elements(gas)-1 do begin ;;only for H2O (ngas=0) and O2 (ngas=1)

;	path = 'GEMSTOOL_07mar2017/GEMSTOOL_physicsdata/LUT_Xsections_data/'
;	path = '/data2/OCO2/'
;	path1 = path + 'ABSCO_binary_files/'
	; path1 = './ABSCO_V500_binary_files'
;	path1 = './ABSCO_V420_binary_files'
;	path1 = './ABSCO_V500_O2AB_binary_files'
	path1 = '/home/mchoi/OSSE_tool/RTM/LUT_Xsections_data_for_all/ABSCO_V500_O2AB_binary_files'
...
;	crs(*,2,*,*) = crs2(*,*,*)
	...
	; wn_res = 0.002
	; wn_res = wn_interval_read;0.01
	wn_res = 0.01
	; wn_res = 0.02
	nwn = fltarr(3) & nwn(*)=!values.f_nan

	for nband = 0, 1 do begin 					;; 1) FOR
...
;				wn_index1 = value_locate(im_wavenumber, wn[0])
				...
				; if ngas eq 1 then h2o_index 	= interpol(num_h2o, h2o_range, im_o2(n))

				fac = 1.
				; if ngas eq 1 then begin ;---original ngas of 1 = CO2
				...
				; 	endif
				; endif
				; if ngas eq 2 then begin ;---original ngas of 2 = O2
...
;---copy to other RTM folders

      for input_loop = 1L, 9 do begin
...
; openw, 5, '../../GEMSTOOL_NSW_wrapper/user_nwn.input'
; for k=0, 1 do printf, 5, nwn[k], format='(i0.0)'
; close, 5
