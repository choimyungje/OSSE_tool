...
private
public SS_Geometry_1, RegularPS_sphergeom
contains
...
!  -----
      ...
      real(ffp)       :: salpha, calpha, calpha1
      real(ffp), parameter :: zero = 0.0d0
...
!  Other variables
   ...
   real(ffp)  ::  dist, distmid, theta_0, theta_1, stheta_0, stheta_1, ground_R, sunpaths(maxlayers)
...
!    Only go down to Initial (LOS-path) Critical layer 
   ...
   do while ( trawl .and.( (Ncrit.eq.0.and.n.lt.nlayers).or.(NCrit.ne.0.and.n.lt.NCrit) ) )
      n = n + 1
      do_ZeroSunBOA = (n.eq.nlayers.and.theta_boa.eq.0.0_ffp)
      if ( doNadir ) then
         theta_n = theta_boa_R ; stheta_n = s0
      else
         theta_n = ground_R - alpha(n) ; stheta_n = sin(theta_n)
      endif
      call FindSunPaths_D (do_ZeroSunBOA,Maxlayers,radii(n),Radii,theta_n,stheta_n,n,sunpaths)
...
!  inputs
      ...
      real(ffp), intent(InOut) :: theta_boa, phi_boa
...
!  Local
      ...
      integer       :: n, j, k
      real(ffp)     :: SolarDirection(3), Radstart
...
!  Inputs
   ...
   real(ffp) , Intent(in)    :: Radius,SolarDirection(3),CumAngle
