...
US1976_file = './PTH_us_standard_1976_res100m_max.dat'
discoveraq_file = './DiscoverAQ_profile_1.dat'

readcol, US1976_file, num_us, alt_us, Temp_us, Pres_us
...
;alt_fin = [findgen(80)*0.25, 80*0.25+findgen(21)*1.00] ;200 m resolution, 51 levels.

; alt_node = [0.0, 0.20, 15.00, 41.00]
; alt_interval = [0.02, 0.20, 2.0]
alt_node = [0.0, 15.00, 70.00]
alt_interval = [0.2, 5.00]
alt_N = bytarr(n_elements(alt_node)-1) & alt_N(*)=!values.f_nan

for i=0L, n_elements(alt_node)-2 do begin
alt_N(i)=round((alt_node(i+1)-alt_node(i))/alt_interval(i))
endfor

alt_fin = [alt_node(0)+findgen(alt_N(0))*alt_interval(0), $
		   alt_node(1)+findgen(alt_N(1))*alt_interval(1), $
		   alt_node(2)]
; alt_fin = [alt_node(0)+findgen(alt_N(0))*alt_interval(0), $
; 		   alt_node(1)+findgen(alt_N(1))*alt_interval(1), $
; 		   alt_node(2)+findgen(alt_N(2))*alt_interval(2), $
; 		   alt_node(3)]
...
spawn, 'cp ./'+file_title +' ../'
spawn, 'cp ./'+file_title +' ../../GAS_PROFILES/'
...
spawn, 'cp ./NSW_heightgrid_'+file_title0+'.dat ../'
