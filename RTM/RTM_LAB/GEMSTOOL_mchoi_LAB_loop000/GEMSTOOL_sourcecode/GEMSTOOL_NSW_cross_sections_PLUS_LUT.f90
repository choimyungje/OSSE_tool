module GEMSTOOL_NSW_cross_sections_PLUS_LUT_m
...
public  :: GEMSTOOL_NSW_CROSS_SECTIONS_PLUS_LUT
private :: reverse_1, fpk

contains

SUBROUTINE GEMSTOOL_NSW_CROSS_SECTIONS_PLUS_LUT  &
          ( MAXWAVNUMS, MAXLAYERS, MAXGASES, CO2_PPMV_MIXRATIO,     & ! INPUT
            NLAYERS, NWAVNUMS, LEVEL_TEMPS, LEVEL_PRESS, WAVNUMS,   & ! INPUT
            NGASES, WHICH_GASES, LUT_PATH, DO_TDERIV, DO_PDERIV,    & ! INPUT  !! Y.Jung
...
!  Gases
   ...
   DO G = 1, NGASES
      ...
      if ( do_TDERIV .or. do_PDERIV ) then
            ...
            call get_lut_crs_plus &
                  ( LUT_path, the_molecule, nwavnums, wavnums(1:nwavnums), is_wavenum,                  & ! input
                    nlevels, atm_press(1:nlevels), level_temps(0:nlayers), do_PDERIV, do_TDERIV,        & ! input
                    GAS_XSECS    (1:nwavnums,0:nlayers,g), & ! Output
                    dGAS_XSECS_dT(1:nwavnums,0:nlayers,g), & ! Output
                    dGAS_XSECS_dP(1:nwavnums,0:nlayers,g))
                   !errstat, message_hitran )                ! Exception handling
            !   write(*,*)'Done LUT calculation for gas = '//which_gases(g)

      else

        call get_lut_crs &
            ( LUT_path, the_molecule, nwavnums, wavnums(1:nwavnums),                &
              is_wavenum, nlevels, atm_press(1:nlevels), level_temps(0:nlayers),    &
              GAS_XSECS(1:nwavnums,0:nlayers,g)) 
              !, errstat, message_hitran )!,!dummy(1:nwavnums,1:nlevels)!
!              print*, g,'get_lut_crs  ____________'
!              print*, wavnums(1:10), GAS_XSECS(1:10,10,g)
              ! write(*,*)'Done LUT calculation for gas = '//which_gases(g)
...
!  FInish
  ...
  RETURN
END SUBROUTINE GEMSTOOL_NSW_CROSS_SECTIONS_PLUS_LUT
...
End module GEMSTOOL_NSW_cross_sections_PLUS_LUT_m
