...
CONTAINS
  ...
  subroutine eval_poly(x, c, norder, value, x0)

    implicit none
    !
    ! expand polynomial using Horner's method.
    !
    ! arguments
    !
    ! x      i  expansion variable
    ! x0     i  expansion point (default=0)
    ! c      i  polynomial coefficients
    ! order  i  order of polynomial coefficients

    ! value  o  polynomial's value at (x-x0)
    !
    real(fpk),    dimension(:), intent(in) :: x
    real(fpk),    intent(inout), optional :: x0
    real(fpk),    dimension(:), intent(in) :: c
    real(fpk),    dimension(:), intent(out) :: value
    integer, intent(in) :: norder
    
    ! local variables
    !
    ! delx   - differences fxrom reference temperature
    ! numx   - number of pts
    !
    real(fpk),    dimension(:), allocatable :: delx
    integer :: i, numx
    ...
    value = 0.d0
!    IF(NOT(present(x0))) x0 = 0.d0
    delx(:) = x(:) - x0
    do i=norder+1, 1, -1
       value(:) =  (c(i)+(value(:)*delx(:)))
    end do
...
!       if (h.eq.0.0d0) pause
  ...
  SUBROUTINE LINTP2 (N1,X1,Y1,N2,X2,Y2)
      ...
      INTEGER        ::     N1,N2
      REAL(fpk)      :: X1(N1),Y1(N1),X2(N2),Y2(N2)
