   ...
   if (skip_vlidort) then
      ...
      if ( do_Profile_Jacobians .or.do_AerOpdepProfile_Jacobians ) then        !  profile Jacobians, also Surface
        CALL vlidort_LPS_master( &
           ...
           VLIDORT_LinSup,   &
           VLIDORT_LinOut )
      else                                   !  Column (Tshift/Surfp) Jacobians, also surface 
...
subroutine GEMSTOOL_RTCALC_PLUS_Final &
   (  MAXWAV, MaxAlbCoeffs, maxgases, maxmessages, W, do_SphericalAlbedo,        & ! GEMSTOOL Dimensions and W
      do_Profile_Jacobians, do_AerOpdepProfile_Jacobians, do_Surface_Jacobians,  & ! GEMSTOOL Jacobian control
      do_Tshift_Jacobian,   do_Surfp_Jacobian, do_normalized_wfoutput,           & ! GEMSTOOL Jacobian control
      do_gases, do_gas_wfs, ngases, nlayers, nstokes, n_geometries, dir,         & ! Control numbers/gases
      Albedo, Tshift, SurfPress, Level_Vmrs, AERTAU_UNSCALED,                    & ! necessary for normalization issues
      ...
      RADIANCE_GASWFS, RADIANCE_AERPROFWFS, RADIANCE_ALBEDOWFS,                  & ! Main program, GEMSTOOL Jacobians Output
      RADIANCE_TSHIFTWFS, RADIANCE_SURFPWFS,                                     & ! Main program, GEMSTOOL Jacobians Output
...
!  GEMSTOOL wavenumber, ClosureCoeffs, gases and MAX-Messages dimensions
   ...
   integer, intent(in) :: MAXWAV, MAXALBCOEFFS, maxgases, maxmessages
...
!  Local
   ...
   integer             :: nm, v, o1, n, n1, q, qprof, nc, g
   character(LEN=3)    :: c3
...
!  Column Jacobians (T-shift and/or Surface pressure). VLIDORT results = NORMALIZED VALUE.
