...
!  status check
      ...
      SUBROUTINE LRRS_STATUS ( STATUS_INPUTCHECK, STATUS_CALCULATION )
...
!  --------------------
      ...
      INTEGER, INTENT(IN) :: STATUS_INPUTCHECK
      INTEGER, INTENT(IN) :: STATUS_CALCULATION

      IF ( STATUS_INPUTCHECK .EQ. LRRS_WARNING ) THEN
        WRITE(*,*)' WARNING: Bad input from LRRS input check'
        WRITE(*,*)'  ------ LRRS has executed with internal defaults'
        WRITE(*,*)'  ------ Check the Logfile for details'
      ELSE IF ( STATUS_INPUTCHECK .EQ. LRRS_SERIOUS ) THEN
        WRITE(*,*)' FATAL: Bad input from LRRS input check'
        WRITE(*,*)'  ------ LRRS will not execute'
        WRITE(*,*)'  ------ Check the Logfile for details'
      ELSE
!        WRITE(*,*)' SUCCESS with LRRS input check'
      ENDIF

      IF ( STATUS_INPUTCHECK .NE. LRRS_SERIOUS ) THEN
       IF ( STATUS_CALCULATION .NE. LRRS_SUCCESS ) THEN
        WRITE(*,*)' FATAL: LRRS execution failure'
        WRITE(*,*)'  ------ Check the Logfile for details'
       ELSE
!        WRITE(*,*)' SUCCESS with LRRS execution'
       ENDIF
      ENDIF
...
!  module, dimensions and numbers
      ...
      USE LRRS_PARS, ONLY: MAX_STREAMS, MAX_2_STREAMS, ZERO, ONE
