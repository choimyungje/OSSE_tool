...
! ###############################################################
      ...
      SUBROUTINE RAMAN_CONVERGE &
       ( DO_UPWELLING, DO_DNWELLING, DO_DOUBLE_CONVTEST, &
         DO_SSCORR_OUTGOING, DO_SSFULL, DO_RAYLEIGH_ONLY, &
         DO_NO_AZIMUTH, DO_FILLING, DO_ELASTIC_ONLY, FOURIER, &
         NPOINTS_LOCAL, N_LOUTPUT, N_OUT_STREAMS, LOCAL_N_USERAZM, &
         N_CONVTESTS, N_GEOMETRIES, GEOM_OFFSETS, AZMFAC, ACCURACY, &
...
!C  Upwelling and downwelling flags
...
!  Single scattering ONLY (SSFULL is flagged). Copy and return
!  -----------------------------------------------------------

      IF ( DO_SSCORR_OUTGOING .AND. DO_SSFULL ) THEN
                ...
                DO I = 1, N_OUT_STREAMS
                 ...
                 DO S = 1, NPOINTS_LOCAL
                  ...
                  IF ( TAZM .NE. ZERO ) THEN
                    IF ( DABS(TAZM/TNEW) .LT. ACCURACY ) THEN
                      COUNT(S) = COUNT(S) + 1
                    ENDIF
                  ELSE
                    COUNT(S)  = COUNT(S) + 1
