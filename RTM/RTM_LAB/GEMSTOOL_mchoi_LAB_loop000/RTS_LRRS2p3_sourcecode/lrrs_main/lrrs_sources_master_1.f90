...
! ###############################################################
      ...
      SUBROUTINE SOURCES_MASTER_1 &
       ( FOURIER, ACTUAL_POINT,DO_ENERGY_BALANCING, &
         ...
         STERM_LAYERMASK_UP, STERM_LAYERMASK_DN, FLUXES_RANKED, &
...
!  Whole layer Greens function Discrete ordinate solutions
        ...
        CALL GREENFUNC_SOLUTION_1 &
            ( FOURIER, N, Z, DO_MULTIPLIER, LRRS_FGSMALL, &
              NSTREAMS, QUAD_WEIGHTS, DELTAUS, &
              XPOS, KEIGEN, KTRANS, EIGENNORM_SAVE, &
              PICUTOFF, FLIPPER, ASOURCE, DELTRANS, INITRANS, &
              QSOURCE_QPOS, QSOURCE_QNEG, &
              RAMAN_WUPPER, RAMAN_WLOWER, &
              ATERM_SAVE, BTERM_SAVE, &
              MULT_M, MULT_P )

!        write(*,555)n,raman_wupper(5,n),raman_wlower(5,n)
!        if ( n.eq.18)pause
! 555    format(i5,1p2e20.10)
!  Debug
!      if ( n.eq.2 ) then
!        write(68,*)z,n
!       write(68,'(i4,1p8e19.10)')n,( RAMAN_WLOWER(i,n)*INITRANS,i=1,8)
!       write(68,'(i4,1p8e19.10)')n,( RAMAN_WLOWER(i+8,n)*INITRANS,i=1,8
!       write(68,'(i4,1p8e19.10)')n,( RAMAN_WUPPER(i,n)*INITRANS,i=1,8)
!       write(68,'(i4,1p8e19.10)')n,( RAMAN_WUPPER(i+8,n)*INITRANS,i=1,8
!        pause
!      endif
...
!  Call to the post processing routine
        ...
        CALL POSTPROCESSING_1 &
          ( DO_UPWELLING, DO_DNWELLING, &
            DO_MSMODE_LRRS, DO_SSCORR_OVERALL, &
            FOURIER, N, NSTREAMS, N_USER_STREAMS, &
            STERM_LAYERMASK_UP(N), STERM_LAYERMASK_DN(N), &
            LRRS_FGSMALL, TRANS_USERM, DELTAUS, USER_STREAMS, &
            FLIPPER, CHANGE_EXPONENT, Z, &
...
!  Debug
!      if (n.eq.1)write(*,*)z
!         write(*,'(i4,1p8e19.10)')n,(RAMAN_WLOWER(i,n),i=1,8)
!         write(*,'(i4,1p8e19.10)')n,(RAMAN_WLOWER(i+8,n),i=1,8)
!         write(*,'(i4,1p8e19.10)')n,(RAMAN_WUPPER(i,n),i=1,8)
!         write(*,'(i4,1p8e19.10)')n,(RAMAN_WUPPER(i+8,n),i=1,8)
!      if ( n.eq.nlayers)pause
...
!         write(*,*)n,z
!        if(z.eq.4)write(91,5)n,raman_wupper(5,n),raman_wlower(5,n)
!        if ( n.eq.18)pause
! 5      format(i5,1p2e20.10)
...
!  End of module
      ...
      END MODULE lrrs_sources_master_1
