...
!      USE LRRS_PARS_m, Only : LDU
      ...
      SUBROUTINE L_ELASTIC_MASTER_1 &
       ( DO_ELASTIC_ONLY, DO_ELASTIC_POSTPROCESSING, DO_MSMODE_LRRS,           &
         DO_INCLUDE_MVOUT, DO_INCLUDE_SURFACE, DO_SSCORR_OVERALL,              &
         DO_UPWELLING, DO_DNWELLING, DO_BIN_REALIZATION, DO_PLANE_PARALLEL,    &
         DO_PROFILE_WFS, DO_COLUMN_WFS, DO_SURFACE_WFS, DO_SLEAVE_WFS,         &
         NKSTORAGE, NKSTORAGE2,                                                &
...
!  Exception handling
        ...
        IF ( FAIL ) THEN
          WRITE(C2,'(I2)')N
          MESSAGE = 'L_Elastic master, call to HOMOG_SOLUTION, layer'//C2
          WRITE(C3, '(I3)' ) CPT
...
!    - Equally valid for profile or column linearization
        ...
        IF ( DO_ATMOS_WFS ) THEN
          ...
          IF ( FAIL ) THEN
            WRITE(C2,'(I2)')N
            MESSAGE = 'L_Elastic master, call to LPC_HOMOG_SOLUTION, layer'//C2
            WRITE(C3, '(I3)' ) CPT
            POINT_TRACE = 'point number: '//C3
            RETURN
          ENDIF
...
!  Exception handling for this solution
        ...
        IF ( FAIL ) THEN
          WRITE(C2,'(I2)')N
          MESSAGE = 'L_Elastic master, BEAM_SOLUTION_ELASTIC, layer'//C2
          WRITE(C3, '(I3)' ) CPT
          POINT_TRACE = 'point number: '//C3
          RETURN
        ENDIF
        ...
        IF ( DO_ATMOS_WFS ) THEN
          ...
          CALL LPC_BEAM_SOLUTION_ELASTIC &
            ( N, FOURIER, DO_PLANE_PARALLEL, NMOMENTS, NSTREAMS, & ! Inputs
              ...
              L_L0_WPARTIC(1,1,1,CPT), FAIL, MESSAGE_SUB )         ! Outputs

          IF ( FAIL ) THEN
            WRITE(C2,'(I2)')N
            MESSAGE = 'L_Elastic master, LPC_BEAM_SOLUTION_ELASTIC, layer'//C2
            WRITE(C3, '(I3)' ) CPT
            POINT_TRACE = 'point number: '//C3
            RETURN
          ENDIF
...
!  (Error tracing)
        ...
        IF ( INFO .NE. 0 ) THEN
          FAIL = .TRUE.
          WRITE(C3, '(I3)' ) CPT
          POINT_TRACE = 'Elastic Radiances/Jacobians, point number: '//C3
          RETURN
        ENDIF
...
! mick fix 4/22/11 - added "(CPT)" to FL1 and FL2
         IF ( DO_UPWELLING ) THEN
...
!  Post processing of the Mean-Intensity & Flux Fields (Azimuth-integrat
...
!    Single layer case (SCOL2). Version 2.5, 9/11/15
           ...
           IF ( NLAYERS .GT. 1 ) THEN
             ...
             IF ( INFO .LT. 0 ) THEN
               ...
               MESSAGE = 'DGBTRS: argument i illegal value, for i '//C2
               WRITE(C3, '(I3)' ) CPT
               POINT_TRACE = 'Elastic Atmospheric Linearization, point number: '//C3
               RETURN
             ENDIF
...
!         if ( do_post_processing)pause'end all Jacs'
...
!    Single layer case (SCOL2). Version 2.5, 9/11/15
         ...
         IF ( NLAYERS .GT. 1 ) THEN
           ...
           IF ( INFO .LT. 0 ) THEN
             ...
             MESSAGE = 'DGBTRS: argument i illegal value, for i = '//C2
             WRITE(C3, '(I3)' ) CPT
             POINT_TRACE = 'Elastic, Surface Linearization, point number: '//C3
             RETURN
           ENDIF
...
!  ################################
       ...
       IF ( DO_SLEAVE_WFS ) THEN
         ...
         IF ( STATUS_SUB /= LRRS_SUCCESS ) THEN
           FAIL = .TRUE.
           MESSAGE = 'L_Elastic master, LRRS_LSSL_WFS'
           WRITE(C3, '(I3)' ) CPT
           POINT_TRACE = 'point number: ' // C3 // TRIM(POINT_TRACE)
           RETURN
         ENDIF
       ENDIF
