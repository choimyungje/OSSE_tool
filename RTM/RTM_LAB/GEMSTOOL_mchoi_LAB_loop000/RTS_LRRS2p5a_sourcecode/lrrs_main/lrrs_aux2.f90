...
! ###############################################################
! #                                                             #
! #  This is lrrs_aux2.f90 - a collection of utility            #
! #  modules compiled from a variety of sources. This is the    #
! #  only part of the LRRS package not written by the Author.   #
! #  The subroutines in lrrs_aux2 are listed below with their   #
! #  source of origin (order of appearance).                    #
! #                                                             #
! #      lcstring:              E. Mikusch, DLR 1994            #
! #      len_string:            E. Mikusch, DLR 1994            #
! #      lrrs_write_status:     M. Christi, 2016                #
! #                               replaced lrrs_status          #
...
! #      lrrs_error_trace:      R.J.D. Spurr, RTS, 2006         #
! #                                                             #
! ###############################################################
...
!  status check
      ...
      SUBROUTINE LRRS_WRITE_STATUS &
        ( ERRORFILE, ERRORUNIT, OPENFILEFLAG, LRRS_Status )

!  Module, dimensions and numbers

      USE LRRS_pars_m, only : MAX_MESSAGES, LRRS_SUCCESS, &
                              LRRS_WARNING, LRRS_SERIOUS
...
!  LRRS status
      ...
      TYPE(LRRS_Exception_Handling), intent(in) :: LRRS_Status
...
!  Exception handling for Model Calculation. New code, 18 May 2010
      ...
      INTEGER             :: N_MESSAGES, STATUS_CALCULATION
...
!  Copy inputs to local variables
      ...
      STATUS_INPUTCHECK               = LRRS_Status%STATUS_INPUTCHECK
      NCHECKMESSAGES                  = LRRS_Status%NCHECKMESSAGES
      CHECKMESSAGES(0:NCHECKMESSAGES) = LRRS_Status%CHECKMESSAGES(0:NCHECKMESSAGES)
      ACTIONS(0:NCHECKMESSAGES)       = LRRS_Status%ACTIONS(0:NCHECKMESSAGES)

      STATUS_CALCULATION = LRRS_Status%STATUS_CALCULATION
      N_MESSAGES         = LRRS_Status%N_MESSAGES
      MESSAGES           = LRRS_Status%MESSAGES

!  Overall status

      STATUS_OVERALL = ( STATUS_INPUTCHECK  .NE. LRRS_SUCCESS ) .OR. &
                       ( STATUS_CALCULATION .NE. LRRS_SUCCESS )
...
!  Check status and write messages
      ...
      IF ( STATUS_INPUTCHECK .EQ. LRRS_WARNING ) THEN
        WRITE(W,*)' WARNING: Suspicious input from LRRS input check'
        WRITE(W,*)'  ------ LRRS has executed with internal defaults'
        ...
        ENDDO
      ELSE IF ( STATUS_INPUTCHECK .EQ. LRRS_SERIOUS ) THEN
        WRITE(W,*)' FATAL:   Wrong input from LRRS input check '
        WRITE(W,*)'  ------ LRRS will not execute'
        WRITE(W,*)'  ------ Here are the messages and actions '
        WRITE(W,'(A,I3)')'    ** Number of messages = ',NCHECKMESSAGES
        DO N = 1, NCHECKMESSAGES
          WRITE(W,'(A,I3,A,A)')'Message # ',N,' : ',adjustl(trim(CHECKMESSAGES(N)))
...
!  Model calculation status
      ...
      IF ( STATUS_INPUTCHECK .NE. LRRS_SERIOUS ) THEN
        IF ( STATUS_CALCULATION .NE. LRRS_SUCCESS ) THEN
          WRITE(W,*)' FATAL: LRRS execution failure'
          WRITE(W,*)'  -----Here are the messages : '
          DO N = 1, N_MESSAGES
            WRITE(W,'(A,A)')' Message : ', TRIM(ADJUSTL(MESSAGES(N)))
          ENDDO
        ENDIF
      ENDIF
...
!  Finish
      ...
      RETURN
      END SUBROUTINE LRRS_WRITE_STATUS
...
!  module, dimensions and numbers
      ...
      USE LRRS_PARS_M, ONLY: MAX_STREAMS, MAX_2_STREAMS, ZERO, ONE
...
!  input/output arguments
      ...
      REAL(FPK), INTENT(INOUT) :: AAD(MAX_STREAMS,MAX_STREAMS)
!      REAL(FPK), INTENT(INOUT) :: WKD(2*MAX_2_STREAMS)
      REAL(FPK), INTENT(INOUT) :: WKD(MAX_2_STREAMS)

      INTEGER  , INTENT(OUT)   :: IER
      REAL(FPK), INTENT(OUT)   :: EVALD(MAX_STREAMS)
      REAL(FPK), INTENT(OUT)   :: EVECD(MAX_STREAMS,MAX_STREAMS)
...
590               CONTINUE
                  EVECD(I,J) = Z
600            CONTINUE
!mick f90 fix - added separate continue
601         CONTINUE
         END IF
...
!
!     .. Parameters ..
      REAL(FPK), PARAMETER :: ONE = 1.0D+0, ZERO = 0.0D+0
...
!
      ...
      SUBROUTINE LRRS_ERROR_TRACE &
           (  ERROR, PAR_STR, NMESS, MESS )
...
!  subroutine arguments
      ...
      INTEGER, INTENT(INOUT) ::           NMESS
!mick fix 10/19/2015 - changed MESS dimension
      !CHARACTER (LEN=*), INTENT(INOUT) :: MESS(100)
      CHARACTER (LEN=*), INTENT(INOUT) :: MESS ( MAX_MESSAGES )
...
!  finish
      ...
      RETURN
      END SUBROUTINE LRRS_ERROR_TRACE

!  End Module

      END MODULE lrrs_aux2_m
