...
!  Added 9/7/18. OMP input and output

      SUBROUTINE ELASTIC_MASTER_1  ( DO_TIMING, NTHREADS,                     & ! OMP Input
...
!  OMP timing, additional output 9/7/17
...
!mick fix 9/7/2017 - define APT below relative to CPT. OMP usage!
...
!mick fix 9/7/2017 - added DO_QUAD_RESULTS, M
...
!$OMP     NPOINTS_LOCAL, NSTR2, NTOTAL, N_SUBDIAG, N_SUPDIAG, R2, DELFAC,      & ! Inputs (Internal)
!$OMP     FLUX_MULTIPLIER,  DO_ELASTIC_SCATTERING, DO_REFLECTED_DIRECTBEAM,    & ! Inputs (Internal)
!$OMP     DO_QUAD_RESULTS, M,                                                  & ! Inputs (Internal)
!$OMP     FOURIER, COS_SZA, TAYLOR_SMALL, TAYLOR_ORDER, FLUX_FACTOR,           & ! Inputs
!$OMP     OFFSET_INNER, NPOINTS_INNER, W_EXCIT,                                & ! Inputs
...
!$OMP     DELTAU_VERT_INPUT, OMEGAMOMS_ELASTIC, SAVE_TRANS_USERM,              & ! Inputs
...
!$OMP     PLM_00_PXI, PLM_00_MXI, PLM_00_PXUI, PLM_00_MXUI,                    & ! Inputs
...
!  Obtain and display total number of threads and local thread
   ...
   IF (TID == 0) THEN
      ...
      write(*,'(1x,a,i1,a)') 'Running Elastic_Fourier with ',OMP_NTHREADS,' thread(s)'
   END IF
...
!  error handling. Modified for OMP usage, 9/7/17.
        ...
        IF ( FAIL_SUB ) THEN
          WRITE(C2,'(I2)')N   ; MESSAGE     = 'Elastic_Master, HOMOG_SOLUTION, layer'//C2
...
!     ALso create the Linearized Matrix................
        ...
        CALL BEAM_SOLUTION_MATRIX &
              ( N, NSTREAMS, EIGENMAT,                    & ! Input
                ...
                QMAT, QPIVOT, FAIL_SUB, MESSAGE_SUB )       ! Output

!  Exception handling. Modified for OMP usage, 9/7/17.

        IF ( FAIL_SUB ) THEN
          WRITE(C2,'(I2)')N   ; MESSAGE     = 'Elastic_Master, BEAM_SOLUTION_MATRIX, layer'//C2
          WRITE(C3,'(I3)')CPT ; POINT_TRACE = 'point number: '//C3
          FAIL = .true. ; GOTO 69
        ENDIF
...
!  Exception handling for this solution. Modified for OMP usage, 9/7/17.

        IF ( FAIL_SUB ) THEN
          WRITE(C2,'(I2)')N   ; MESSAGE     = 'Elastic_Master, BEAM_SOLUTION_ELASTIC, layer'//C2
          WRITE(C3,'(I3)')CPT ; POINT_TRACE = 'point number: '//C3
          FAIL = .true. ; GOTO 69
        ENDIF
...
!  (Error tracing). Modified for OMP usage, 9/7/17.
        ...
        IF ( INFO .ne. 0 ) THEN
          WRITE(C3,'(I3)')CPT ; POINT_TRACE = 'Elastic_Master Radiances, point number: '//C3
          FAIL = .true. ; GOTO 69
        ENDIF
...
!mick fix 9/7/2017 - define APT here relative to CPT. OMP Usage   !!!!!
...
!mick fix 9/7/2017 - define APT above relative to CPT
...
!  End post processing clause for Intensity.
