...
!  error handling
          ...
          IF ( FAIL ) THEN
            WRITE(C2,'(I2)')LAYER
            MESSAGE = 'Fourier master, call to HOMOG_SOLUTION, layer'//C2
            WRITE(C3, '(I3)' ) CPT
...
!  (Error tracing)
        ...
        IF ( INFO .NE. 0 ) THEN
          FAIL = .TRUE.
          WRITE(C3, '(I3)' ) CPT
          POINT_TRACE = 'Raman Fourier Radiances, point number: '//C3
          RETURN
        ENDIF
...
!  (Error tracing)
        ...
        IF ( INFO .GT. 0 ) THEN
          ...
          FAIL = .TRUE.
          WRITE(C3, '(I3)' ) CPT
          POINT_TRACE = 'Raman Fouirer Radiances, point number: '//C3
          RETURN
        ENDIF
