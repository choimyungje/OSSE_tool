...
!  Copy BRDF inputs. 11//4/17. First line is new.

      DO_BRDF_Wav1           = BRDF_Sup_In%BS_DO_BRDF_Wav1         ! New 11/4/17
               ...
               DO I = 1, NSTREAMS
                 DO J = 1, NSTREAMS
                   BRDF_F(M,I,J) = BRDF_F(M,I,J) &
                      + BRDF_FACTORS(K,L) * LOCAL_BRDF_F(I,J)
                 ENDDO
               ENDDO
...
!        11/4/17. Introduce wavelength dimension in factors
               ...
               DO I = 1, NSTREAMS
                 BRDF_F_0(M,I) = BRDF_F_0(M,I) &
                       + BRDF_FACTORS(K,L) * LOCAL_BRDF_F_0(I)
               ENDDO
                 ...
                 DO UM = 1, N_USER_STREAMS
                   DO J = 1, NSTREAMS
                     USER_BRDF_F(M,UM,J) = USER_BRDF_F(M,UM,J) &
                        + BRDF_FACTORS(K,L) * LOCAL_USER_BRDF_F(UM,J)
                   ENDDO
                 ENDDO
               ENDIF
               ...
               IF ( DO_USER_STREAMS ) THEN
                 DO UM = 1, N_USER_STREAMS
                   USER_BRDF_F_0(M,UM) = USER_BRDF_F_0(M,UM) &
                          + BRDF_FACTORS(K,L) * LOCAL_USER_BRDF_F_0(UM)
                 ENDDO
               ENDIF
