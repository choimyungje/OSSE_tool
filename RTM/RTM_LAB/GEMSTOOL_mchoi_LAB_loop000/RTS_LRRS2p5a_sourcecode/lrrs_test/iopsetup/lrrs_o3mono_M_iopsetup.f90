...
!   -- Rob mod 5/12/17 for 2p5a, Included Accessory Module for phase function calculations
      ...
      MODULE O3MONO_M_iopsetup_NoLin_m
...
!  Auxiliary and workhorse modules
      ...
      PRIVATE
      PUBLIC :: LRRS_IOPSETUP_O3MONO_M

      CONTAINS

      SUBROUTINE LRRS_IOPSETUP_O3MONO_M ( &
...
!   -- Rob mod 5/12/17 for 2p5a, removed MAX_MOMENTS_INPUT
      ...
      USE LRRS_IO_DEFS_m
      USE LRRS_RAMAN_SPECTROSCOPY_m, Only : LRRS_BUFFERING_MONO
...
!      - Get the binning limits for the outer window
      ...
      IF ( DO_MONO_REALIZATION ) THEN
            CALL LRRS_BUFFERING_MONO &
               ( MAX_INPUT_POINTS, .FALSE.,     & !Inputs
                 INPUT_LAMBDAS, INPUT_FLUXES,       & !Inputs
                 LAMBDA_START, LAMBDA_FINISH,       & !Inputs
                 N_INPUT_POINTS, LAMBDAS_RANKED, FLUXES_RANKED)   !Outputs
...
!  End
      ...
      RETURN
      END SUBROUTINE LRRS_IOPSETUP_O3MONO_M

!  End module

      END MODULE O3MONO_M_iopsetup_NoLin_m
