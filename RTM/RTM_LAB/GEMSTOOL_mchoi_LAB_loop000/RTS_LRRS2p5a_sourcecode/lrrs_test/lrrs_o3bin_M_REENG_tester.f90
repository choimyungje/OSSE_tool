...
!  Input data for LRRS code control (USER-DEFINED INPUT)
      ...
      CALL LRRS_READ_INPUTS ( &
        'lrrs_test/lrrs_o3bin_M_REENG_tester.cfg', &
...
!      time to prepare the optical property inputs.
      ...
      write(*,'(1x,i5,8x,i1,7x,f6.2)') &
        NPOINTS_INNER, NCORES, StdTime/REAL(TIME_DIVIDER)
...
!  Setup output files
      ...
      IF ( DO_ENERGY_BALANCING ) THEN
        FILNAM36 = 'lrrs_test/results_o3bin_M_REENGT_EB_SZA**_lamb.resg'
      ELSE
        FILNAM36 = 'lrrs_test/results_o3bin_M_REENGT_CR_SZA**_lamb.resg'
      ENDIF
...
!  Rob add core indicator

      IF ( NCORES.eq.1 ) filnam36(52:54) = '_C1'
      IF ( NCORES.eq.2 ) filnam36(52:54) = '_C2'
      IF ( NCORES.eq.4 ) filnam36(52:54) = '_C4'
...
!  Display timing (off for LRRS2.5a comparison)
      ...
      !CALL CPU_TIME(E2)
      !WRITE(*,*)
      !WRITE(*,'(8X,1X,A,F6.2,A)') 'Test time = ',(E2-E1)/REAL(NCORES),' sec'
