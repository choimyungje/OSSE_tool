module Sioris_Raman_m

  use Sioris_Raman_Specdata_m

  public  Sioris_Raman_Compute, Sioris_Raman_Getspec
  private BSPLINE, SPLINE, SPLINT, Sioris_Raman
...
SUBROUTINE Sioris_Raman_Compute &
        (nz, nw, sza, vza, sca, albedo, do_upwelling, RamanData, ts, rhos, &
         wave, sol, taus, rspec, problems, message )
  ...
  TYPE(Sioris_Raman_Specdata), INTENT (IN)       :: RamanData
...
!  Initialize Exception handling
  ...
  IF (nuhi - nulo + 1 > MAXNU) THEN
     message  = 'Sioris_Raman_Compute: Need to increase MAXNU!!!'
     problems = .TRUE.; RETURN
  ELSE IF (nuhi <= nulo) THEN
     message  = 'Sioris_Raman_Compute: nulo>=nuhi, should never happen!!!'
     problems = .TRUE.; RETURN
  ENDIF
  ...
  DO i = 1, nz
     ...
     IF (errstat < 0) THEN
        write(c1,'(I1)')errstat
        message  =  'Sioris_Raman_Compute: Initial BSPLINE error on STRANS, errstat = '//c1
        problems = .TRUE.; RETURN
     ENDIF
     ...
     IF (errstat < 0) THEN
        write(c1,'(I1)')errstat
        message  = 'Sioris_Raman_Compute: Initial BSPLINE error on VTRANS, errstat = '//c1
        problems = .TRUE.; RETURN
     ENDIF
  ENDDO
  
  ! Call raman program
  CALL SIORIS_RAMAN(nulo, nuhi, nu, nz, sca, tmpalb, RamanData, ts, rhos, st(1:nu,1:nz), vt(1:nu,1:nz), ring(1:nu))
  ...
  IF (errstat < 0) THEN
     write(c1,'(I1)')errstat
     message  = 'Sioris_Raman_Compute: Final BSPLINE error on RSPEC, errstat = '//c1
     problems = .TRUE.; RETURN
  ENDIF
  
  RETURN
  
END SUBROUTINE Sioris_Raman_Compute
...
SUBROUTINE Sioris_Raman(nulo, nuhi, nline, nz, sca, albedo, RamanData, T, rhos, R, tran, ring)
  ...
  DO iz = 1, nz
     ...
     DO j = 0, N2Jmax
        IF ( ifix(10* (j / 2.0 - ifix( j / 2.0) ) ) == 5) THEN
           N2pop(j) = 3 * ( 2 * j + 1.0d0) * EXP(-c1 * RamanData%N2E(j) / T(iz)) 
        ELSE
           N2pop(j) = 6 * ( 2 * j + 1.0d0) * EXP(-c1 * RamanData%N2E(j) / T(iz))
        ENDIF
        ZN2= ZN2 + N2pop(j)  
     ENDDO
     ...
     DO j = 0, N2Jmax - 2 
        N2csec(j) = temp * N2pop(j + 2) * RamanData%N2b(j) /  ZN2
     ENDDO
  
     ! calculate static part of Stokes cross sections for N2      
     DO j = N2Jmax - 1, 2 * N2Jmax - 3 
        N2csec(j) = temp * N2pop(j - N2Jmax + 1) * RamanData%N2b(j) / ZN2
     ENDDO
  
     ! calculate state sum for O2 
     ZO2 = 0
     DO k = 0, O2maxJ
        O2popz(k) = ( 2 * RamanData%O2JZ(k) + 1 ) * EXP(- c1 * RamanData%O2EnZ(k) / T(iz) )
        ZO2 = ZO2 + O2popz(k)
     ENDDO
       
     ! O2 cross sections
     DO k = 0, 2 * O2max-7
        O2pop(k) = (2 * RamanData%O2J2(k) + 1 ) * EXP(-c1 * RamanData%O2E(k) / T(iz) )
        O2csec(k)= temp * O2pop(k) * RamanData%O2b(k) / ZO2
     ENDDO
     ...
     DO nu = fidx, lidx
        ...
        DO j = 0, 2 * N2Jmax-3
           N2so(nu) = N2so(nu) + N2csec(j) * (REAL(nu - RamanData%N2shift(j)))**4 * temp1
           N2sumin(nu) = N2sumin(nu) + R(nu + RamanData%N2shift(j), iz) / R(nu, iz) &
                * N2csec(j) * gammaN2( nu + RamanData%N2shift(j)) ** 2 * temp2
        ENDDO
     
        temp1 = gammaO2(nu) * gammaO2(nu)
        DO k = 0, 2 * O2max-7           
           O2so(nu) = O2so(nu) + O2csec(k) * (REAL(nu - RamanData%O2shift(k)))**4 * temp1
           O2sumin(nu) = O2sumin(nu) + R(nu + RamanData%O2shift(k), iz) / R(nu, iz) &
                * O2csec(k) * gammaO2(nu + RamanData%O2shift(k)) ** 2 * temp2
        ENDDO
...
END SUBROUTINE Sioris_Raman

subroutine Sioris_Raman_Getspec (RamanData)

  IMPLICIT NONE

  TYPE(Sioris_Raman_Specdata), INTENT (INOUT)       :: RamanData
  ...
  REAL (KIND=dp), DIMENSION(0:2*O2max-7)   :: O2E,  O2b
  INTEGER, DIMENSION (0:O2maxJ)            :: O2JZ
  INTEGER, DIMENSION (0:2*O2max-7)         :: O2J2, O2shift
  INTEGER, DIMENSION (0:2*N2Jmax-3)        :: N2shift  

!mick fix 11/10/2016 - assigned dp values to elements of the dp arrays N2E, N2b, O2EnZ, O2E, & O2b

data N2E / &
   0.0000_dp,    3.9791_dp,   11.9373_dp,   23.8741_dp,   39.7892_dp, &
  59.6821_dp,   83.5521_dp,  111.3983_dp,  143.2197_dp,  179.0154_dp, &
 218.7839_dp,   262.524_dp,  310.2341_dp,  361.9126_dp,  417.5576_dp, &
 477.1673_dp,  540.7395_dp,  608.2722_dp,  679.7628_dp,   755.209_dp, &
 834.6081_dp,  917.9574_dp, 1005.2540_dp, 1096.4948_dp, 1191.6766_dp, &
1290.7963_dp, 1393.8503_dp, 1500.8350_dp, 1611.7467_dp /

data N2b / &
0.2000_dp, 0.2571_dp, 0.2857_dp, 0.3030_dp, 0.3147_dp, 0.3231_dp, 0.3294_dp, 0.3344_dp, &
0.3383_dp, 0.3416_dp, 0.3443_dp, 0.3467_dp, 0.3487_dp, 0.3504_dp, 0.3519_dp, 0.3532_dp, &
0.3544_dp, 0.3555_dp, 0.3565_dp, 0.3573_dp, 0.3581_dp, 0.3589_dp, 0.3595_dp, 0.3601_dp, &
0.3607_dp, 0.3612_dp, 0.3617_dp, 1.0000_dp, 0.6000_dp, 0.5143_dp, 0.4762_dp, 0.4545_dp, &
0.4406_dp, 0.4308_dp, 0.4235_dp, 0.4180_dp, 0.4135_dp, 0.4099_dp, 0.4070_dp, 0.4044_dp, &
0.4023_dp, 0.4004_dp, 0.3988_dp, 0.3974_dp, 0.3961_dp, 0.3950_dp, 0.3940_dp, 0.3931_dp, &
0.3922_dp, 0.3915_dp, 0.3908_dp, 0.3902_dp, 0.3896_dp, 0.3890_dp /

data N2shift / &
 -12,  -20,  -28,  -36,  -44,  -52,  -60,  -68,  -76,  -84, -91, -99, &
...
data O2JZ / &
0, 2, 1, 2, 4, 3, 4, 6, 5, 8, 6, 7, 10, 8, 9,       &
12, 10, 11, 14, 12, 13, 16, 14, 15, 18, 16, 17, 20, &
18, 19, 22, 20, 21, 24, 22, 23, 26, 24, 25, 28, 26, &
27, 30, 28, 29, 32, 30, 31, 34, 32, 33, 36, 34, 35 /

data O2EnZ / &
   0.0000_dp,    2.0843_dp,    3.9611_dp,   16.2529_dp,   16.3876_dp,   18.3372_dp, &
  42.2001_dp,   42.2240_dp,   44.2117_dp,   79.5646_dp,   79.6070_dp,   81.5805_dp, &
 128.3978_dp,  128.4921_dp,  130.4376_dp,  188.7135_dp,  188.8532_dp,  190.7749_dp, &
 260.5011_dp,  260.6826_dp,  262.5829_dp,  343.7484_dp,  343.9697_dp,  345.8500_dp, &
 438.4418_dp,  438.7015_dp,  440.5620_dp,  544.5658_dp,  544.8628_dp,  546.7050_dp, &
 662.1030_dp,  662.4368_dp,  664.2610_dp,  791.0344_dp,  791.4045_dp,  793.2100_dp, &
 931.3390_dp,  931.7450_dp,  933.5330_dp, 1082.9941_dp, 1083.4356_dp, 1085.2060_dp, &
1245.9750_dp,  246.4518_dp, 1248.2040_dp, 1420.2552_dp, 1420.7672_dp, 1422.5020_dp, &
1605.8064_dp, 1606.3533_dp, 1608.0710_dp, 1802.5983_dp, 1803.1802_dp, 1804.8810_dp /

data O2J2 /  &
4, 19, 17, 1, 15, 13, 11, 9, 3, 7, 5, 5, 7, 9, 11, 13, 3, 2, 15, 17, 19, &
2, 2, 4, 3, 2, 3, 4, 4, 6, 5, 4, 5, 6, 8, 7, 6, 7, 4, 8, 10, 9, 8, 9,    &
10, 12, 11, 10, 11, 12, 14, 13, 12, 13, 14, 16, 15, 14, 15, 16, 18, 17,  &
16, 17, 18, 20, 19, 18, 19, 22, 21, 20, 24, 23, 22, 26, 25, 24, 28, 27,  &
26, 30, 29, 28, 32, 31, 30, 34, 33, 32, 2, 18, 16, 2, 14, 12, 10, 8,     &
4, 6, 6, 4, 8, 10, 12, 14, 2, 0, 16, 18, 20, 1, 2, 2, 1, 0, 2, 3, 4,     &
4, 3, 2, 4, 5, 6, 5, 4, 6, 2, 7, 8, 7, 6, 8, 9, 10, 9, 8, 10, 11, 12,    &
11, 10, 12, 13, 14, 13, 12, 14, 15, 16, 15, 14, 16, 17, 18, 17, 16, 18,  &
19, 20, 19, 18, 20, 22, 21, 20, 24, 23, 22, 26, 25, 24, 28, 27, 26, 30,  &
29, 28, 32, 31, 30 /

data O2E /  &
  16.3876_dp,  546.7050_dp,  440.5620_dp,    3.9611_dp,  345.8500_dp,  262.5829_dp,  190.7749_dp,  130.4376_dp, &
  18.3372_dp,   81.5805_dp,   44.2117_dp,   44.2117_dp,   81.5805_dp,  130.4376_dp,  190.7749_dp,  262.5829_dp, &
  18.3372_dp,    2.0843_dp,  345.8500_dp,  440.5620_dp,  546.7050_dp,   16.2529_dp,   16.2529_dp,   16.3876_dp, &
  18.3372_dp,   16.2529_dp,   18.3372_dp,   42.2001_dp,   42.2001_dp,   42.2240_dp,   44.2117_dp,   42.2001_dp, &
  44.2117_dp,   79.6070_dp,   79.5646_dp,   81.5805_dp,   79.6070_dp,   81.5805_dp,   42.2001_dp,  128.4921_dp, &
 128.3978_dp,  130.4376_dp,  128.4921_dp,  130.4376_dp,  188.8532_dp,  188.7135_dp,  190.7749_dp,  188.8532_dp, &
 190.7749_dp,  260.6826_dp,  260.5011_dp,  262.5829_dp,  260.6826_dp,  262.5829_dp,  343.9697_dp,  343.7484_dp, &
 345.8500_dp,  343.9697_dp,  345.8500_dp,  438.7015_dp,  438.4418_dp,  440.5620_dp,  438.7015_dp,  440.5620_dp, &
 544.8628_dp,  544.5658_dp,  546.7050_dp,  544.8628_dp,  546.7050_dp,  662.1030_dp,  664.2610_dp,  662.4368_dp, &
 791.0344_dp,  793.2100_dp,  791.4045_dp,  931.3390_dp,  933.5330_dp,  931.7450_dp, 1082.9941_dp, 1085.2060_dp, &
1083.4356_dp, 1245.9750_dp, 1248.2040_dp, 1246.4518_dp, 1420.2552_dp, 1422.5020_dp, 1420.7672_dp, 1605.8064_dp, &
1608.0710_dp, 1606.3533_dp,   16.2529_dp,  544.8628_dp,  438.7015_dp,    2.0843_dp,  343.9697_dp,  260.6826_dp, &
 188.8532_dp,  128.4921_dp,   16.3876_dp,   79.6070_dp,   42.2240_dp,   42.2001_dp,   79.5646_dp,  128.3978_dp, &
 188.7135_dp,  260.5011_dp,   16.2529_dp,    0.0000_dp,  343.7484_dp,  438.4418_dp,  544.5658_dp,    3.9611_dp, &
   2.0843_dp,    2.0843_dp,    3.9611_dp,    0.0000_dp,    2.0843_dp,   18.3372_dp,   16.3876_dp,   16.3876_dp, &
  18.3372_dp,   16.2529_dp,   16.3876_dp,   44.2117_dp,   42.2240_dp,   44.2117_dp,   42.2001_dp,   42.2240_dp, &
   2.0843_dp,   81.5805_dp,   79.5646_dp,   81.5805_dp,   79.6070_dp,   79.5646_dp,  130.4376_dp,  128.3978_dp, &
 130.4376_dp,  128.4921_dp,  128.3978_dp,  190.7749_dp,  188.7135_dp,  190.7749_dp,  188.8532_dp,  188.7135_dp, &
 262.5829_dp,  260.5011_dp,  262.5829_dp,  260.6826_dp,  260.5011_dp,  345.8500_dp,  343.7484_dp,  345.8500_dp, &
 343.9697_dp,  343.7484_dp,  440.5620_dp,  438.4418_dp,  440.5620_dp,  438.7015_dp,  438.4418_dp,  546.7050_dp, &
 544.5658_dp,  546.7050_dp,  544.8628_dp,  544.5658_dp,  662.1030_dp,  664.2610_dp,  662.4368_dp,  791.0344_dp, &
 793.2100_dp,  791.4045_dp,  931.3390_dp,  933.5330_dp,  931.7450_dp, 1082.9941_dp, 1085.2060_dp, 1083.4356_dp, &
1245.9750_dp, 1248.2040_dp, 1246.4518_dp, 1420.2552_dp, 1422.5020_dp, 1420.7672_dp /

data O2b /  &
0.000207_dp, 0.002045_dp, 0.00255_dp,  0.2308_dp,   0.003268_dp, 0.004338_dp, 0.006036_dp, 0.008967_dp, &
0.05132_dp,  0.0147_dp,   0.02207_dp,  0.02842_dp,  0.01222_dp,  0.007753_dp, 0.005353_dp, 0.003916_dp, &
0.0769_dp,   0.1077_dp,   0.002989_dp, 0.002356_dp, 0.001905_dp, 0.1615_dp,   0.017752_dp, 0.262699_dp, &
0.1714_dp,   0.09234_dp,  0.06596_dp,  0.04342_dp,  0.001458_dp, 0.304466_dp, 0.2727_dp,   0.259873_dp, &
0.02613_dp,  0.01979_dp,  0.32365_dp,  0.3077_dp,   0.303719_dp, 0.01387_dp,  0.00103_dp,  0.01127_dp,  &
0.33465_dp,  0.3251_dp,   0.323348_dp, 0.008577_dp, 0.007271_dp, 0.341777_dp, 0.3345_dp,   0.334499_dp, &
0.005822_dp, 0.005076_dp, 0.346767_dp, 0.3422_dp,   0.341691_dp, 0.004209_dp, 0.003743_dp, 0.3504_dp,   &
0.3471_dp,   0.346713_dp, 0.003184_dp, 0.002874_dp, 0.3532_dp,   0.3506_dp,   0.35042_dp,  0.002492_dp, &
0.002275_dp, 0.3555_dp,   0.3534_dp,   0.3533_dp,   0.002079_dp, 0.3573_dp,   0.3556_dp,   0.3589_dp,   &
0.3589_dp,   0.3574_dp,   0.3574_dp,   0.3601_dp,   0.3589_dp,   0.3589_dp,   0.3612_dp,   0.3602_dp,   &
0.3602_dp,   0.3622_dp,   0.3613_dp,   0.3613_dp,   0.363_dp,    0.3622_dp,   0.3622_dp,   0.3637_dp,   &
0.363_dp,    0.363_dp,    0.000373_dp, 0.002156_dp, 0.002705_dp, 0.1385_dp,   0.003493_dp, 0.004685_dp, &
0.00661_dp,  0.010022_dp, 0.03991_dp,  0.01696_dp,  0.01867_dp,  0.03474_dp,  0.01078_dp,  0.007014_dp, &
0.004924_dp, 0.003646_dp, 0.1077_dp,   0.5383_dp,   0.002808_dp, 0.002229_dp, 0.001812_dp, 0.2692_dp,   &
0.017752_dp, 0.4729_dp,   0.4_dp,      0.4617_dp,   0.09234_dp,  0.05583_dp,  0.001458_dp, 0.439784_dp, &
0.4286_dp,   0.467772_dp, 0.03193_dp,  0.02339_dp,  0.423234_dp, 0.4196_dp,   0.438706_dp, 0.016_dp,    &
0.001854_dp, 0.01278_dp,  0.413391_dp, 0.4118_dp,   0.422839_dp, 0.009586_dp, 0.008037_dp, 0.406877_dp, &
0.406_dp,    0.413205_dp, 0.006377_dp, 0.005517_dp, 0.40225_dp,  0.4017_dp,   0.406775_dp, 0.004546_dp, &
0.00402_dp,  0.398795_dp, 0.3985_dp,   0.402187_dp, 0.003403_dp, 0.003059_dp, 0.3961_dp,   0.3959_dp,   &
0.3988_dp,   0.002643_dp, 0.002405_dp, 0.393979_dp, 0.3938_dp,   0.3961_dp,   0.002112_dp, 0.001941_dp, &
0.3922_dp,   0.3921_dp,   0.394_dp,    0.001726_dp, 0.3908_dp,   0.3907_dp,   0.3922_dp,   0.3895_dp,   &
0.3895_dp,   0.3908_dp,   0.3885_dp,   0.3885_dp,   0.3896_dp,   0.3876_dp,   0.3876_dp,   0.3885_dp,   &
0.3868_dp,   0.3868_dp,   0.3876_dp,   0.3861_dp,   0.3861_dp,   0.3868_dp /

data O2shift /  &
0, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2, -2,   &
-12, -14, -14, -14, -16, -16, -24, -26, -26, -26, -26, -28, -35, -37, -37, -37, -39, &
-40, -47, -49, -49, -49, -51, -58, -60, -60, -60, -62, -70, -72, -72, -72, -74, -81, &
-83, -83, -83, -85, -93, -95, -95, -95, -97, -104, -106, -106, -106, -108, -118,     &
-118, -118, -129, -129, -129, -140, -140, -140, -152, -152, -152, -163, -163, -163,  &
-174, -174, -174, -186, -186, -186,                                                  &
0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2,                       &
12, 14, 14, 14, 16, 16, 24, 26, 26, 26, 26, 28, 35, 37, 37, 37, 39,                  &
40, 47, 49, 49, 49, 51, 58, 60, 60, 60, 62, 70, 72, 72, 72, 74, 81,                  &
83, 83, 83, 85, 93, 95, 95, 95, 97, 104, 106, 106, 106, 108, 116, 118,               &
118, 118, 120, 129, 129, 129, 140, 140, 140, 152, 152, 152, 163, 163, 163,           &
174, 174, 174, 186, 186, 186 /
...
!  Fill
   ...
   RamanData%N2E     = N2E
   RamanData%N2b     = N2b
   RamanData%N2shift = N2shift

   RamanData%O2E     = O2E
   RamanData%O2b     = O2b
   RamanData%O2JZ    = O2JZ
   RamanData%O2J2    = O2J2
   RamanData%O2EnZ   = O2EnZ
   RamanData%O2shift = O2shift

!  Finish

   return
end subroutine Sioris_Raman_Getspec
