...
! Myungje Choi 25 April 2019
!   * Optimization for O2A and B band only
!   * Original ABSCO LUT resolution : 0.01 cm-1
!   * Interpolation to target resolution and wavenumber
!   * only working for NSW mode (wavenumber mode), not for UVN mode now
...
REAL(KIND=dp), DIMENSION(31001, nz) :: crsdt_read1 !mchoi; maximum dimension:31001 for O2A/O2B
REAL(KIND=dp), DIMENSION(31001, nz) :: crsdp_read1 !mchoi; maximum dimension:31001 for O2A/O2B

REAL(KIND=dp), DIMENSION(25001, nz) :: crs_read2 !mchoi; maximum dimension:31001 for O2A/O2B
REAL(KIND=dp), DIMENSION(25001, nz) :: crsdt_read2 !mchoi; maximum dimension:31001 for O2A/O2B
REAL(KIND=dp), DIMENSION(25001, nz) :: crsdp_read2 !mchoi; maximum dimension:31001 for O2A/O2B
...
crsdp_read1 = 0.0d0 !mchoi
crsdt_read1 = 0.0d0 !mchoi

crs_read2 = 0.0d0 !mchoi
crsdp_read2 = 0.0d0 !mchoi
crsdt_read2 = 0.0d0 !mchoi
...
! !50 format(100000e20.8)
    ...
    !---case 1 : O2A
    open(11, file = crs_filename1, access = 'direct', form = 'unformatted', recl = nwn1*nz*4*dp)
    read(11, rec=1) ((crs_read1(i,j),i=1,nwn1),j=1,nz) !mchoi-modified
    close(11)

    open(12, file = crsdt_filename1, access = 'direct', form = 'unformatted', recl = nwn1*nz*4*dp)
    read(12, rec=1) ((crsdt_read1(i,j), i=1,nwn1), j=1,nz)
    close(12)

    open(13, file = crsdp_filename1, access = 'direct', form = 'unformatted', recl = nwn1*nz*4*dp)
    read(13, rec=1) ((crsdp_read1(i,j), i=1,nwn1), j=1,nz)
    close(13)

    !---case 2 : O2B
    open(11, file = crs_filename2, access = 'direct', form = 'unformatted', recl = nwn2*nz*4*dp)
    read(11, rec=1) ((crs_read2(i,j),i=1,nwn2),j=1,nz) !mchoi-modified
    close(11)

    open(12, file = crsdt_filename2, access = 'direct', form = 'unformatted', recl = nwn2*nz*4*dp)
    read(12, rec=1) ((crsdt_read2(i,j), i=1,nwn2), j=1,nz)
    close(12)

    open(13, file = crsdp_filename2, access = 'direct', form = 'unformatted', recl = nwn2*nz*4*dp)
    read(13, rec=1) ((crsdp_read2(i,j), i=1,nwn2), j=1,nz)
    close(13)
    ...
    if (lambda(i) .ge. wstart_read1 .and. lambda(i) .lt. wend_read1) then
      ...
      crsdt(i,:) = fa1 * crsdt_read1(wa1,:) + fa2 * crsdt_read1(wa2, :)
      crsdp(i,:) = fa1 * crsdp_read1(wa1,:) + fa2 * crsdp_read1(wa2, :)
    ...
    !--case 2: within O2B band  
      ...
      crsdt(i,:) = fa1 * crsdt_read2(wa1,:) + fa2 * crsdt_read2(wa2, :)
      crsdp(i,:) = fa1 * crsdp_read2(wa1,:) + fa2 * crsdp_read2(wa2, :)
