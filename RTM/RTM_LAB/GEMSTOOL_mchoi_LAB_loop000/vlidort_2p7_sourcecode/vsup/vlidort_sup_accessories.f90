...
!  Rob Fix 3/18/15. Wavelength checks introduced.
...
!  Rob Fix 3/18/15. VBRDF Wavelength for the NewCM Glint Model
...
!  Rob Fix 3/18/15. VLIDORT optical Wavelength
...
!  VLIDORT Modified User Value inputs
      ...
      ATMOS_WAVELENGTH = VLIDORT_FixIn%Optical%TS_ATMOS_WAVELENGTH         ! Rob Fix 3/18/15
...
!  Rob Fix 3/18/15. Check wavelengths, NewCM Glint only

      IF ( BS_DO_NewCMGLINT ) THEN
         if ( BS_WAVELENGTH .ne. ATMOS_WAVELENGTH ) THEN
            NM = NM + 1
...
!  Rob Fix 3/18/15. VSLEAVE Wavelength for the NewCM Glint Model
...
!  Rob Fix 3/18/15. Check wavelengths, 

      IF ( SL_DO_FLUORESCENCE ) THEN
         if ( SL_FL_WAVELENGTH .ne. ATMOS_WAVELENGTH * 1000.0d0 ) THEN
            NM = NM + 1
         ...
         if ( SL_WAVELENGTH .ne. ATMOS_WAVELENGTH ) THEN
            NM = NM + 1
...
!  Rob Fix 3/18/15. Check wavelength, NewCM Glint only + Water leaving

      IF ( BS_DO_NewCMGLINT ) THEN
         if ( BS_WAVELENGTH .ne. SL_WAVELENGTH ) THEN
            NM = NM + 1
            MESSAGES(NM) = 'NewCM Glint: Wavelength does not agree with Water-leaving wavelength'
