...
!  Input F755 Amplitude and Flag for using Data Gaussian parameters
...
!   --> Flag for Linear parameterization, linear parameters, Jacobian flags

      logical   :: FL_Do_LinearParam
      REAL(FPK) :: FL_ScalingConstant, FL_ScalingGradient
      Logical   :: FL_Do_ScalCons_Jac, FL_Do_ScalGrad_Jac
...
!     GEMSTOOL Alteration 11/30/16. Additional linear parameterization variables
      ...
      FL_Do_ScalCons_Jac = .false.
      FL_Do_ScalGrad_Jac = .false.
...
!     GEMSTOOL Alteration 11/30/16. Additional linear parameterization variables
        ...
        IF ( FL_DO_LinearParam ) then
          IF ( DO_SL_JACOBIANS .and. DO_ISO_JACOBIANS ) THEN
            PAR_STR = 'Do Jacobians for FL Scaling Constant value?'
            IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) &
               READ (FILUNIT,*,ERR=998) FL_DO_SCALCONS_JAC
            CALL FINDPAR_ERROR ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )
            PAR_STR = 'Do Jacobians for FL Scaling Gradient value?'
            IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) &
              READ (FILUNIT,*,ERR=998) FL_DO_SCALGRAD_JAC
            CALL FINDPAR_ERROR ( ERROR, PAR_STR, STATUS, NM, MESSAGES, ACTIONS )
            IF ( FL_DO_SCALCONS_JAC ) QM = QM + 1
            IF ( FL_DO_SCALGRAD_JAC ) QM = QM + 1
          ENDIF
        ELSE
          IF ( DO_SL_JACOBIANS .and. DO_ISO_JACOBIANS ) THEN
...
!  GEMSTOOL Alteration 11/30/16. Additional linear parameterization variables
      ...
      VSLEAVE_LinSup_In%SL_FL_Do_ScalCons_Jac  = FL_Do_ScalCons_Jac
      VSLEAVE_LinSup_In%SL_FL_Do_ScalGrad_Jac  = FL_Do_ScalGrad_Jac
...
!
      ...
      SUBROUTINE VSLEAVE_LIN_MAINMASTER ( Datapath, &
...
!  Input F755 Amplitude,  Flag for using Data Gaussian parameters
!     Linearization of Gaussians only when the data option is not set
!  GEMSTOOL Alteration 11/30/16. re-listing of variables
...
!  GEMSTOOL Alteration 11/30/16. Copy linear parameterization variables
      ...
      FL_DO_SCALCONS_JAC  = VSLEAVE_LinSup_In%SL_FL_DO_SCALCONS_JAC
      FL_DO_SCALGRAD_JAC  = VSLEAVE_LinSup_In%SL_FL_DO_SCALGRAD_JAC
...
!    GEMSTOOL Alteration 11/30/16. For Gaussian option, amplitude scaling done here, not later.
          ...
          if ( FL_Do_LinearParam ) then
            Lamda = 1.0d0 - ( FL_Wavelength / 755.0_fpk )
            FsSum = FL_ScalingConstant + FL_ScalingGradient * lamda          
          else
            FsSum = zero
            do k = 1, 2
...
!   FF is the fudge factor
          ...
          help2 = FF * Fs755(IB) / FL_SunSpec
          Fluor_ISOTROPIC = help2 * FsSum
...
!    GEMSTOOL Alteration 11/30/16. Linear parameterization introduced as alternative..
          ...
          NSLPARS = 0
          if ( DO_ISO_JACOBIANS ) then
            if ( FL_Do_LinearParam ) then
              IF ( FL_Do_ScalCons_Jac ) then
                NSLPARS = NSLPARS + 1 ; VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(NSLPARS,1,IB) = help2
              ENDIF
              IF ( FL_Do_ScalGrad_Jac ) then
                NSLPARS = NSLPARS + 1 ; VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(NSLPARS,1,IB) = help2 * lamda
              ENDIF
            endif
          endif

!    GEMSTOOL Alteration 11/30/16. Gaussian jacobians now optional
!       Apply Main linearization (w.r.t Fs755 Amplitude)
!       Apply Gaussian linearizations if flagged.

          if ( DO_ISO_JACOBIANS ) then
            if ( FL_F755_JACOBIANS ) then
              NSLPARS = NSLPARS + 1 ; help1 = Fluor_ISOTROPIC / FL_Amplitude755
              VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(1,1,IB) = help1
            endif
...
!   GEMSTOOL Alteration 11/30/16. Fill in the other variables
          ...
          if ( NSLPARS.gt.0 ) then
            IF ( DO_EXACT .or. DO_EXACTONLY ) then
              do m = 1, NSLPARS
                 VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(m,1,1:n_user_streams,1:n_user_relazms,IB) = &
                      VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(m,1,IB)
              enddo
            ENDIF
            if ( .not. DO_EXACTONLY ) then
              do m = 1, NSLPARS
                VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0      (m,0,1,1:nstreams,      ib) = &
                      VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(m,1,IB)
                VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0 (m,0,1,1:n_user_streams,ib) = &
                      VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(m,1,IB)
              enddo
            endif
          endif
...
!   Pigment
         ...
         if ( DO_CHLORCONC_WF ) then
           Q = Q + 1
           do ib = 1, nbeams
             VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(q,1,ib) = dC_WLeaving_ISO(ib)
             IF ( DO_EXACT .or. DO_EXACTONLY ) then
               if ( do_Isotropic ) then
                  VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(q,1,1:n_user_streams,1:n_user_relazms,IB) = dC_WLeaving_ISO(ib)
               else
                  do um = 1, n_user_streams
             ...
             if ( .not. DO_EXACTONLY ) then
               if ( do_Isotropic ) then
                  VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0      (q,0,1,1:nstreams,      ib) = dC_WLeaving_ISO(ib)
                  VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0 (q,0,1,1:n_user_streams,ib) = dC_WLeaving_ISO(ib)
               else
                  do um = 1, n_user_streams
                     VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0 (q,0,1,um,ib) =  dC_WLeaving_SV(ib,um)
                  enddo
                  do um = 1, nstreams
                     VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0(q,0,1,um,ib) = dC_WLeaving_SD(ib,um)
                  enddo
               endif
             endif
           enddo
...
!   Windspeed
         ...
         if ( DO_WINDSPEED_WF ) then
           Q = Q + 1
           do ib = 1, nbeams
             VSLEAVE_LinSup_Out%SL_LS_SLTERM_ISOTROPIC(q,1,ib) = dW_WLeaving_ISO(ib)
             IF ( DO_EXACT .or. DO_EXACTONLY ) then
               if ( do_Isotropic ) then
                  VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(q,1,1:n_user_streams,1:n_user_relazms,IB) = dW_WLeaving_ISO(ib)
               else
                  do um = 1, n_user_streams
             ...
             if ( .not. DO_EXACTONLY ) then
               if ( do_Isotropic ) then
                  VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0      (q,0,1,1:nstreams,      ib) = dW_WLeaving_ISO(ib)
                  VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0 (q,0,1,1:n_user_streams,ib) = dW_WLeaving_ISO(ib)
               else
                  do um = 1, n_user_streams
                     VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0 (q,0,1,um,ib) =  dW_WLeaving_SV(ib,um)
                  enddo
                  do um = 1, nstreams
                     VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0(q,0,1,um,ib) = dW_WLeaving_SD(ib,um)
                  enddo
               endif
             endif
           enddo
