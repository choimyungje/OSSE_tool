...
!      FUNCTION solar_spec_irradiance(wavelength) result(ssi)
      FUNCTION solar_spec_irradiance(wavelength,datapath) result(ssi)
...
!Function by Mick Christi - 16 July 2012
      ...
      !Input variable, non-standard, added by Rob for GEMSTOOL
...
!Obtain solar data if necessary
 ...
 !         open(unit=50,file='vlidort_v_test/data/wehrli85.dat',status='old',action='read')
          open(unit=50,file=Adjustl(Trim(DataPath))//'wehrli85.dat',status='old',action='read')
...
!          open(unit=50,file='vlidort_v_test/data/ref_solar_irradiance_whi-2008_ver2.dat',status='old',action='read')
          open(unit=50,file=Adjustl(Trim(DataPath))//'ref_solar_irradiance_whi-2008_ver2.dat',status='old',action='read')
